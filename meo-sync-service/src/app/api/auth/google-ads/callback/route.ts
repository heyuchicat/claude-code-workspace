...
import { completeGoogleAdsOAuth } from "@/lib/google-ads-client";
...
export async function GET(request: NextRequest) {
  ...
  if (!code) return redirectWithError("Google Ads連携がキャンセルされました");
  ...
  let result;
  try {
    result = await completeGoogleAdsOAuth(businessId, code);
  } catch (err) {
  ...
  const targetUrl =
    result.status === "needs_selection"
      ? `/businesses/${businessId}/select-google-ads-account?pendingId=${result.pendingId}`
      : `/businesses/${businessId}?connected=google_ads`;
